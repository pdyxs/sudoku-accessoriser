...
describe('Error Handling Integration Flow', () => {
  ...
  beforeAll(() => {
    ...
    global.NotificationManager = class NotificationManager {
      ...
      showError(message) {
        return this.showToast(message, 'error', 0);
      }
      showSuccess(message) {
      ...
      createSudokuPadUrlValidator() {
        return async url => {
          ...
          try {
            ...
            if (
              !['sudokupad.app', 'www.sudokupad.app'].includes(
                parsedUrl.hostname
              )
            ) {
              return {
    ...
    global.PuzzleManager = class PuzzleManager {
      ...
      createUserFriendlyError(originalError, errorType) {
        ...
        const userError = new Error(
          messages[errorType] ||
            `Failed to load puzzle: ${originalError.message}`
        );
  ...
  describe('Network Error Flow', () => {
    ...
    test('should handle 404 errors appropriately', async () => {
      ...
      try {
        await puzzleManager.extractPuzzleData(
          'https://sudokupad.app/nonexistent'
        );
      } catch (error) {
        uiController.showError(error.message);
      }
