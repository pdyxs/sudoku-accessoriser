...
describe('PuzzleManager Error Handling', () => {
        ...
        global.PuzzleManager = class PuzzleManager {
            ...
            createUserFriendlyError(originalError, errorType) {
                const messages = {
                    ...
                    server_error: 'SudokuPad server error. Please try again in a few moments.',
                    parse_error: 'Unable to parse puzzle data. The puzzle format may not be supported.',
    ...
    describe('User-Friendly Error Messages', () => {
        test('should create user-friendly network error', () => {
            ...
            const userError = puzzleManager.createUserFriendlyError(originalError, 'network');
            
            expect(userError.message).toBe('Network connection failed. Please check your internet connection and try again.');
        ...
        test('should create user-friendly timeout error', () => {
            ...
            const userError = puzzleManager.createUserFriendlyError(originalError, 'timeout');
            
            expect(userError.message).toBe('Request timed out. The puzzle server may be slow to respond. Please try again.');
        });

        test('should create user-friendly not found error', () => {
            const originalError = new Error('404');
            const userError = puzzleManager.createUserFriendlyError(originalError, 'not_found');
            
            expect(userError.message).toBe('Puzzle not found. Please check that the URL is correct and the puzzle exists.');
        });

        test('should create user-friendly forbidden error', () => {
            const originalError = new Error('403');
            const userError = puzzleManager.createUserFriendlyError(originalError, 'forbidden');
            
            expect(userError.message).toBe('Access denied. This puzzle may be private or the URL may be incorrect.');
        });

        test('should create user-friendly server error', () => {
            const originalError = new Error('500');
            const userError = puzzleManager.createUserFriendlyError(originalError, 'server_error');
            
            expect(userError.message).toBe('SudokuPad server error. Please try again in a few moments.');
        });

        test('should create user-friendly parse error', () => {
            const originalError = new Error('invalid JSON');
            const userError = puzzleManager.createUserFriendlyError(originalError, 'parse_error');
            
            expect(userError.message).toBe('Unable to parse puzzle data. The puzzle format may not be supported.');
        });

        test('should handle unknown error types', () => {
            const originalError = new Error('mysterious error');
            const userError = puzzleManager.createUserFriendlyError(originalError, 'unknown');
    ...
    describe('Basic Retry Logic', () => {
        test('should succeed on first attempt', async () => {
            ...
            global.PuzzleConverter.convertSudokuPadUrl.mockResolvedValueOnce(mockPuzzleData);
            
            const result = await puzzleManager.extractPuzzleData('https://sudokupad.app/test123');
            
            expect(result).toEqual(mockPuzzleData);
            expect(global.PuzzleConverter.convertSudokuPadUrl).toHaveBeenCalledTimes(1);
        });

        test('should not retry non-retryable errors', async () => {
            const notFoundError = new Error('Puzzle not found (404)');
            
            global.PuzzleConverter.convertSudokuPadUrl.mockRejectedValueOnce(notFoundError);
            
            await expect(puzzleManager.extractPuzzleData('https://sudokupad.app/test123'))
    ...
    describe('Error Handling Integration', () => {
        test('should preserve original error for debugging', async () => {
            ...
            global.PuzzleConverter.convertSudokuPadUrl.mockRejectedValue(originalError);
