...
describe('Curved Lines Support', () => {
  ...
  beforeAll(() => {
    ...
    const curvedFilePath = path.resolve(
      process.cwd(),
      'examples/example-curved.json'
    );
  ...
  describe('Curved Example Data Processing', () => {
    ...
    test('should process curved puzzle data correctly', () => {
      const processedData = PuzzleConverter.processPuzzleData(
        curvedExampleData,
        'curved-example'
      );
  ...
  describe('Line Type Analysis', () => {
    ...
    test('should handle overlay lines correctly', () => {
      ...
      const overlayLines = curvedExampleData.lines.filter(
        line => line.target === 'overlay'
      );
      ...
      if (overlayLines.length > 0) {
        ...
        expect(processedData.totalLines).toBeGreaterThanOrEqual(
          overlayLines.length
        );
      }
    });
  ...
  describe('Color Analysis', () => {
    ...
    test('should handle black overlay lines', () => {
      ...
      const blackFeature = processedData.features.find(
        f => f.visual.color === '#000000'
      );
  ...
  describe('Data Integrity', () => {
    ...
    test('should calculate total lines correctly', () => {
      ...
      const calculatedTotal = processedData.features.reduce(
