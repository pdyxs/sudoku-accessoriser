...
describe('Line Feature Extraction', () => {
  ...
  describe('Sample Data Processing', () => {
    ...
    test('should group lines by color correctly', () => {
      ...
      // Check line counts
      const greenFeature = processedData.features.find(
        f => f.visual.color === '#67f067ff'
      );
      const purpleFeature = processedData.features.find(
        f => f.visual.color === '#f067f0ff'
      );
  ...
  describe('Real Puzzle Integration', () => {
    ...
    test('should convert real puzzle URL and extract line features', async () => {
      const result = await PuzzleConverter.convertSudokuPadUrl(
        'https://sudokupad.app/line-features-test'
      );
    ...
    test('should handle puzzle with no lines', async () => {
      ...
      const result = await PuzzleConverter.convertSudokuPadUrl(
        'https://sudokupad.app/no-lines-test'
      );
    ...
    test('should handle puzzle with empty lines array', async () => {
      ...
      const result = await PuzzleConverter.convertSudokuPadUrl(
        'https://sudokupad.app/empty-lines-test'
      );
