...
class ValidationManager {
  ...
  registerValidator(inputId, validatorFn, options = {}) {
    ...
    const config = {
      ...
      feedbackElement: document.getElementById(
        options.feedbackId || `${inputId}-feedback`
      ),
  ...
  setValidationState(inputId, state, message) {
    ...
    if (config.iconElement) {
      config.iconElement.classList.remove(
        'success',
        'error',
        'validating',
        'show'
   ...
   */
  async validateAll() {
    const validationPromises = Array.from(this.validators.keys()).map(inputId =>
      this.validateInput(inputId)
    );
  ...
  createSudokuPadUrlValidator() {
    return async value => {
      ...
      if (
        invalidPaths.includes(path) ||
        invalidPaths.includes('/' + pathContent)
      ) {
        return {
          isValid: false,
          message:
      ...
      if (
        window.PuzzleConverter &&
        window.PuzzleConverter.isValidSudokuPadUrl
      ) {
