...
class PuzzleConverter {
    ...
    try {
      ...
      if (PuzzleLoader.isRemotePuzzleId(processedPuzzleId)) {
        ...
        try {
          ...
          console.log(
            'Fetched data sample:',
            String(processedPuzzleId).substring(0, 100)
          );
      ...
      if (reFPuzPrefix.test(processedPuzzleId)) {
        ...
        try {
          const stripFPuzPrefix = fpuzzle =>
            fpuzzle.replace(reFPuzPrefix, '$2');
          const decodedUrl = decodeURIComponent(
            stripFPuzPrefix(processedPuzzleId)
          );
        ...
        console.log(
          'Data to parse:',
          String(processedPuzzleId).substring(0, 200)
        );

        // If the data looks compressed (starts with scl), we need to handle it properly
        if (
          typeof processedPuzzleId === 'string' &&
          processedPuzzleId.startsWith('scl')
        ) {
          console.log(
            'Data appears to be SCL compressed, using parsePuzzleData'
          );
      ...
      console.log(
        'Final data sample:',
        JSON.stringify(finalData).substring(0, 200)
      );
  ...
  static processPuzzleData(puzzleData, puzzleId) {
    ...
    return {
      title:
        puzzleData.metadata?.title || puzzleData.title || 'SudokuPad Puzzle',
      puzzleId: puzzleId,
  ...
  static extractLineFeatures(puzzleData) {
    ...
    console.log(
      `Grouped lines into ${Object.keys(colorGroups).length} color groups:`,
    ...
    console.log(
      `Created ${features.length} line features:`,
      features.map(f => `${f.visual.color} (${f.count} lines)`)
    );
